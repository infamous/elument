...
class LumeElement extends Effectful(HTMLElement) {
	 ...
	 */
	dispatchEventWithCall(event: Event) {
		const name = event.type
		const methodName = 'on' + name
		if (methodName in this) (this as any)[methodName](event)
...
type PrefixProps<Prefix extends string, T> = {[K in keyof T as K extends string ? `${Prefix}${K}` : K]: T[K]}
